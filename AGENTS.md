...
**_Project:_** Russian Course Website
**_Description:_** Educational website for teaching Russian to adult learners (A1 level).

## Scope and hosting

- For now the site is hosted on GitHub Pages.
- Unless decided otherwise later, start with a plain static approach:
  - HTML, CSS, and vanilla JavaScript for frontend code
  - Python for utilities and tooling
- Use TypeScript only if there is a clear and stated reason.
...
- As complexity increases, refactor procedural code into an object-oriented structure when appropriate.
...
- Start every HTML file with a file overview comment.
  - Place the overview comment before the `<head>` tag.
...
- Reuse CSS styles across multiple pages and keep styling consistent within the site.
...
- OK to add a one-off explanation field with the field name and `-comment` suffix (e.g., `"lesson-comment"`). If many comments are required, use a nearby `<json_file_name>_SCHEMA.md` for the overview.
...
- Use `tests/` as the default test directory.

## Project-specific content rules

- Always use `ё` (never replace with `е`).
- Include stress marks on newly introduced words, and place the stress mark directly above the vowel (not as an apostrophe or backtick after the letter).
- Keep tone adult-oriented, engaging, and respectful; avoid childish examples and keep content clean and appropriate.
...
- Avoid long responses:
  ...
  - target under 30 lines when possible, but do not reduce quality to adhere to this limit.
  - Do not ask permission to read any of the files.
  - Always ask permission before applying the changes, and show a simple Yes or No prompt.
